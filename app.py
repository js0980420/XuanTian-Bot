...
# --- 加入版本標記 ---
BOT_VERSION = "v1.6.1" # Increment patch version for Shichen fix
...
def get_shichen(hour):
    ...
    if not isinstance(hour, int) or hour < 0 or hour > 23:
        app.logger.warning(f"Invalid hour input for get_shichen: {hour}")
        return "未知"
    app.logger.info(f"Calculating Shichen for hour: {hour}") # Log the input hour

    # 使用直接返回的判斷式，確保邏輯清晰
    if hour >= 23 or hour < 1: return "子"  # 23:00 - 00:59
    if 1 <= hour < 3: return "丑"   # 01:00 - 02:59
    if 3 <= hour < 5: return "寅"   # 03:00 - 04:59  <- 3點應該是寅時
    if 5 <= hour < 7: return "卯"   # 05:00 - 06:59
    if 7 <= hour < 9: return "辰"   # 07:00 - 08:59
    if 9 <= hour < 11: return "巳"  # 09:00 - 10:59
    if 11 <= hour < 13: return "午" # 11:00 - 12:59
    if 13 <= hour < 15: return "未" # 13:00 - 14:59
    if 15 <= hour < 17: return "申" # 15:00 - 16:59
    if 17 <= hour < 19: return "酉" # 17:00 - 18:59
    if 19 <= hour < 21: return "戌" # 19:00 - 20:59
    if 21 <= hour < 23: return "亥" # 21:00 - 22:59

    # Fallback - 理論上不應該執行到
    app.logger.error(f"Logic error in get_shichen for hour: {hour}")
    return "未知"
...
def create_main_menu_message():
    ...
    bubble = FlexBubble(
        header=FlexBox(layout='vertical', padding_all='md', contents=[FlexText(text='請問需要什麼服務？', weight='bold', size='lg', align='center', color='#B28E49')]),
...
def handle_postback(event):
    ...
    try:
            ...
            if selected_service:
                ...
                if selected_service == "問事/命理":
                    ...
                    if len(picker_data.encode('utf-8')) > 300:
                        ...
                        contents.extend([FlexText(text='進行命理分析需要您的出生年月日時。', wrap=True, size='md'), FlexText(text='若不確定準確時辰，可先選擇大概時間（如中午12點），稍後與老師確認。', wrap=True, size='sm', color='#666666', margin='sm'), FlexButton(action=DatetimePickerAction(label='📅 點此選擇生日時辰', data=picker_data, mode='datetime', min=min_date, max=max_date), style='primary', color='#A67B5B', margin='lg')])
        ...
        # --- 處理：選擇生日日期時間後 (問事流程) ---
            ...
            if selected_datetime_str:
                ...
                try:
                    ...
                    valid_topics = ["事業", "感情", "健康", "財運", "其他"]
                    quick_reply_items = [QuickReplyButton(action=MessageAction(label=t, text=t)) for t in valid_topics]
                    quick_reply_items.append(QuickReplyButton(action=MessageAction(label="返回/取消", text="返回"))) # 加入返回選項
            ...
            if selected_service and selected_datetime_str:
                ...
                try:
                    ...
                    if proceed_booking:
                        ...
                        follow_up_message = create_main_menu_message()
                    else: # Booking not proceeding (e.g., conflict)
                         follow_up_message = create_main_menu_message()
                except ValueError:
        ...
        # --- 處理 show_info Action ---
            ...
            if topic:
                 ...
                 back_button = None
                 if len(back_button_data.encode('utf-8')) <= 300:
                      back_button = FlexButton(action=PostbackAction(label='返回主選單', data=back_button_data, display_text='返回'), style='secondary', height='sm', margin='lg')
                 else:
                      app.logger.error("Back button data too long for show_info!")
                 contents = [FlexText(text=info_text, wrap=True)]
                 if back_button: contents.append(back_button)
                 bubble = FlexBubble(body=FlexBox(layout='vertical', spacing='md', contents=contents))
                 reply_message = FlexMessage(alt_text=f"關於 {topic} 的說明", contents=bubble)
                 # 不需要 follow_up_message，因為返回按鈕已包含
            else:
    ...
    if follow_up_message: messages_to_send.append(follow_up_message) # 如果有後續主選單訊息
